...
//显示有符号数字
void OLED_ShowSignedNum(uint8_t x, uint8_t y, int32_t num, uint8_t len, uint8_t size2, uint8_t Color_Turn)
{
	uint8_t i;
	uint32_t Number1;
	...
	if (num >= 0)						                //数字大于等于0
	{
		OLED_ShowChar(x, y, '+', size2, Color_Turn);    //显示+号
		Number1 = num;					            //Number1直接等于num
	}
	else									            //数字小于0
	{
		OLED_ShowChar(x, y, '-', size2, Color_Turn);	//显示-号
		Number1 = -num;					                //Number1等于num取负
	}
	...
	for (i = 0; i < len; i++)			//遍历数字的每一位								
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		OLED_ShowChar(x + (i + 1) * size2, y, Number1 / oled_pow(10, len - i - 1) % 10 + '0', size2, Color_Turn);
	}
}
